...
If($Result){
    Write-Host "FreePhysicalMemory=$($Result.FreePhysicalMemory)"
    Write-Host "FreeSpaceInPagingFiles=$($Result.FreeSpaceInPagingFiles)"
    Write-Host "FreeVirtualMemory=$($Result.FreeVirtualMemory)"
    Write-Host "NumberOfProcesses=$($Result.NumberOfProcesses)"
    Write-Host "SizeStoredInPagingFiles=$($Result.SizeStoredInPagingFiles)"
    Write-Host "TotalVirtualMemorySize=$($Result.TotalVirtualMemorySize)"
    Write-Host "TotalVisibleMemorySize=$($Result.TotalVisibleMemorySize)"
}
Exit 0