...
$WSManParams.Filter = 'SELECT * FROM Win32_PerfRawData_NTDS_NTDS'
$NTDSResult = Get-WSManInstance @WSManParams

If($NTDSResult){
    Write-Host "DRAInboundBytesTotalSinceBoot=$($NTDSResult.DRAInboundBytesTotalSinceBoot)"
    Write-Host "DRAInboundObjectsAppliedPersec=$($NTDSResult.DRAInboundObjectsAppliedPersec)"
    Write-Host "DRAInboundObjectsPersec=$($NTDSResult.DRAInboundObjectsPersec)"
    Write-Host "DRAOutboundObjectsPersec=$($NTDSResult.DRAOutboundObjectsPersec)"
    Write-Host "DRAPendingReplicationSynchronizations=$($NTDSResult.DRAPendingReplicationSynchronizations)"
    Write-Host "DSNAMECACHEHITRATE=$($NTDSResult.DSNAMECACHEHITRATE)"
    Write-Host "DSThreadsinUse=$($NTDSResult.DSThreadsinUse)"
    Write-Host "LDAPBindTime=$($NTDSResult.LDAPBindTime)"
    Write-Host "LDAPClientSessions=$($NTDSResult.LDAPClientSessions)"
    Write-Host "LDAPCLOSEDCONNECTIONSPERSEC=$($NTDSResult.LDAPCLOSEDCONNECTIONSPERSEC)"
    Write-Host "LDAPNEWCONNECTIONSPERSEC=$($NTDSResult.LDAPNEWCONNECTIONSPERSEC)"
    Write-Host "LDAPSearchesPersec=$($NTDSResult.LDAPSearchesPersec)"
    Write-Host "LDAPSuccessfulBindsPersec=$($NTDSResult.LDAPSuccessfulBindsPersec)"
    Write-Host "NTLMBINDSPERSEC=$($NTDSResult.NTLMBINDSPERSEC)"
}
Exit 0