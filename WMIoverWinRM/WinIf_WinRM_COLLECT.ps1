...
$WSManParams = @{
    ...
    Filter = 'SELECT * FROM Win32_PerfRawData_Tcpip_NetworkInterface'
...
If($Result){
    Foreach($Item in $Result){
        ...
        Write-Host "$Wildvalue.PacketsOutboundDiscarded=$($Item.PacketsOutboundDiscarded)"
        Write-Host "$Wildvalue.PacketsReceivedDiscarded=$($Item.PacketsReceivedDiscarded)"
        Write-Host "$Wildvalue.PacketsReceivedNonUnicastPerSec=$($Item.PacketsReceivedNonUnicastPerSec)"
        Write-Host "$Wildvalue.PacketsReceivedUnicastPerSec=$($Item.PacketsReceivedUnicastPerSec)"
        Write-Host "$Wildvalue.PacketsSentNonUnicastPerSec=$($Item.PacketsSentNonUnicastPerSec)"
        Write-Host "$Wildvalue.PacketsSentUnicastPerSec=$($Item.PacketsSentUnicastPerSec)"
    }
}
Exit 0