...
If(!$(Test-Path -Path "C:\Program Files (x86)\Google\Chrome\Application\chromedriver.exe")){
    ...
    If($(Test-Path -Path $ChromePath)){
        ...
        Move-Item -Path "chromedriver_win32\chromedriver.exe" -Destination "C:\Program Files (x86)\Google\Chrome\Application" -Force
...
Else{
    Write-Host "[INFO]:Skipping Chromedrive install since a version is already been detected at: C:\Program Files (x86)\Google\Chrome\Application\chromedriver.exe"
}

#Instal Selenium
$AgentPath = "C:\Program Files (x86)\LogicMonitor\Agent"
If(!$(Test-Path -Path "C:\Program Files (x86)\LogicMonitor\Agent\custom\selenium-server-standalone-3.141.59.jar")){
...
If($(Test-Path -Path $AgentPath) -and !$(Test-Path -Path "$AgentPath/custom" )){
    Write-Host "[INFO]:Creating custom jar folder in LogicMonitor agent directory"
    New-Item -ItemType "Directory" -Path $AgentPath -Name "custom" | Out-Null
}
Move-Item -Path $SeleniumJar -Destination "$AgentPath/custom" -Force
}
Else{
...
Else{
    ...
    If(!$WrapperConf[$NewJarLine]){ #Only append if new line is empty
...
Write-Host "    Selenium JAR File Path: $AgentPath\selenium-server-standalone-3.141.59.jar" -ForegroundColor Green